(()=>{
  ...
  style.textContent=`
    .joined.photo-collection{
      width:300px;height:232px;display:block!important;
    ...
    .joined.photo-collection .object[data-kind="image"]{
      ...
      transition:transform 220ms cubic-bezier(.2,.9,.25,1)!important;
      box-shadow:0 5px 16px rgba(0,0,0,.12)!important;
    }
    .joined.photo-collection .object:nth-of-type(1){z-index:1;transform:translate(-54%,-49%) rotate(-6deg) scale(.9)!important}
    .joined.photo-collection .object:nth-of-type(2){z-index:2;transform:translate(-52%,-50%) rotate(-2deg) scale(.92)!important}
    .joined.photo-collection .object:nth-of-type(3){z-index:3;transform:translate(-49%,-50%) rotate(2.5deg) scale(.93)!important}
    .joined.photo-collection .object:nth-of-type(4){z-index:4;transform:translate(-46%,-49%) rotate(6deg) scale(.91)!important}
    .collection-count{position:absolute;right:-8px;top:-8px;z-index:20;min-width:34px;height:34px;padding:0 9px;border-radius:18px;background:#171717;color:#fff;display:grid;place-items:center;font:700 15px/1 -apple-system,BlinkMacSystemFont,sans-serif;box-shadow:0 4px 12px rgba(0,0,0,.2);pointer-events:none}
  ...
  function mergeIntoCollection(o,g){
    ...
    const gp=center(g);setPos(o,gp.x,gp.y);
    g.appendChild(o);updateCollection(g);
    requestAnimationFrame(()=>g.classList.remove('collection-ready'));
  }

  const observer=new MutationObserver(records=>{
    for(const rec of records){rec.addedNodes.forEach(n=>{if(n.nodeType===1){qualifyJoined(n);n.querySelectorAll?.('.joined').forEach(qualifyJoined)}})}
  });
  ...
  stage.addEventListener('pointerup',e=>{
    ...
    collections().forEach(g=>g.classList.remove('collection-ready'));
    if(!o||o.parentElement!==stage)return;
    const g=nearestCollection(o,205);if(g)setTimeout(()=>mergeIntoCollection(o,g),0);
  },true);

  const touches=new Map();let peel=null;
  ...
  stage.addEventListener('pointermove',e=>{
    ...
    const d=Math.hypot(pts[1].x-pts[0].x,pts[1].y-pts[0].y);if(d-peel.base<48)return;
    peel.done=true;
