(()=>{
  ...
  style.textContent=`
    ...
    .verb-edge{position:absolute;display:flex;align-items:center;justify-content:center;font:700 12px/1 -apple-system,BlinkMacSystemFont,"SF Pro Display",sans-serif;letter-spacing:.12em;text-transform:uppercase;color:rgba(0,0,0,.66);opacity:.56;transition:opacity 140ms ease,transform 140ms ease,filter 140ms ease}
    .verb-edge::before{content:"";position:absolute;background:var(--verb-color);opacity:.62;border-radius:999px;box-shadow:0 0 24px color-mix(in srgb,var(--verb-color) 38%,transparent);transition:opacity 140ms ease,box-shadow 140ms ease,transform 140ms ease}
    ...
    .verb-edge.intent::before{opacity:.92;box-shadow:0 0 38px color-mix(in srgb,var(--verb-color) 58%,transparent);transform:scale(1.12)}
    .verb-edge.commit::before{transform:scale(1.3);box-shadow:0 0 54px color-mix(in srgb,var(--verb-color) 72%,transparent)}
  ...
  const pointerObjects=new Map();
  const INTENT=150,COMMIT=78;
  function isWorkspaceObject(target){return target.closest?.('.object,.joined')||null}
  function clearIntent(){Object.values(edgeEls).forEach(el=>el.classList.remove('intent','commit'))}
  ...
  function move(e){
    ...
    if(hit.distance<INTENT){edgeEls[hit.edge]?.classList.add('intent');if(hit.distance<COMMIT)edgeEls[hit.edge]?.classList.add('commit')}
  }
  function hide(e){
