(()=>{
  ...
  style.textContent=`
    .object[data-kind="image"]{
      width:264px!important;height:202px!important;padding:10px 10px 28px!important;
      border-radius:7px!important;background:#f8f6ef!important;
  ...
  requestAnimationFrame(()=>{
    // The hidden add control lets the original sandbox create fully wired
    // objects without duplicating any of its drag/throw physics here.
    ...
    const positions=[
      [r.width*.34,r.height*.38],
      [r.width*.64,r.height*.36],
      [r.width*.38,r.height*.65],
      [r.width*.67,r.height*.64]
    ];

    objects.forEach((o,i)=>{
      o.dataset.shape='photo';o.dataset.kind='image';o.textContent='';
      o.style.backgroundImage='none';o.style.setProperty('--size','202px');
