(()=>{
  ...
  requestAnimationFrame(()=>{
    ...
    objects=objects.slice(0,4);if(!objects.length)return;
    const r=stage.getBoundingClientRect();
    const positions=[[r.width*.34,r.height*.38],[r.width*.64,r.height*.36],[r.width*.38,r.height*.65],[r.width*.67,r.height*.64]];
    objects.forEach((o,i)=>{o.dataset.shape='photo';o.dataset.kind='image';o.textContent='';o.style.backgroundImage='none';o.style.setProperty('--size','202px');const print=document.createElement('div');print.className='photo-print';print.style.backgroundImage=`url("${photos[i]}")`;o.appendChild(print);const [x,y]=positions[i];o.dataset.x=String(x);o.dataset.y=String(y);o.style.setProperty('--x',x+'px');o.style.setProperty('--y',y+'px')});
    if(add)add.remove();const status=document.getElementById('status');if(status)status.textContent='4 image objects';
  });
})();