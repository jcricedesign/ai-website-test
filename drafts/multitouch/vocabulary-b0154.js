(()=>{
  ...
  function objectEdge(el){const sr=stage.getBoundingClientRect(),r=el.getBoundingClientRect();const d={left:r.left-sr.left,right:sr.right-r.right,top:r.top-sr.top,bottom:sr.bottom-r.bottom};const edge=Object.keys(d).reduce((a,b)=>d[a]<d[b]?a:b);return{edge,distance:d[edge],sr,r}}
  function show(e){if(document.body.classList.contains('transform-mode'))return;const obj=isWorkspaceObject(e.target);if(!obj)return;activePointers.add(e.pointerId);pointerObjects.set(e.pointerId,obj);vocabulary.classList.add('visible');clearIntent()}
  function move(e){const obj=pointerObjects.get(e.pointerId);if(!obj)return;clearIntent();const hit=objectEdge(obj);if(hit.distance>=INTENT)return;const edge=edgeEls[hit.edge];if(!edge)return;const force=Math.max(0,Math.min(1,(INTENT-hit.distance)/INTENT));const axisSize=(hit.edge==='left'||hit.edge==='right')?hit.sr.height:hit.sr.width;const span=Math.min(axisSize*.88,96+force*axisSize*.62);edge.style.setProperty('--edge-span',span+'px');edge.classList.add('intent');if(hit.distance<COMMIT)edge.classList.add('commit')}
  function hide(e){activePointers.delete(e.pointerId);pointerObjects.delete(e.pointerId);clearIntent();if(!activePointers.size)vocabulary.classList.remove('visible')}
  stage.addEventListener('pointerdown',show,true);stage.addEventListener('pointermove',move,true);stage.addEventListener('pointerup',hide,true);stage.addEventListener('pointercancel',hide,true);stage.addEventListener('lostpointercapture',hide,true);
})();
